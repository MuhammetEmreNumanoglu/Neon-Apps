...
export default function DataDemoPage() {
    ...
    // Directly use users data without setState in useEffect
    const [localUsers, setLocalUsers] = useState<User[] | null>(null);

    // Sync localUsers with users data on mount and refetch
    // Wrap in useMemo to prevent dependency issues
    const syncedLocalUsers = useMemo(() => localUsers ?? users ?? [], [localUsers, users]);

    const handleConfirmedBatchDelete = useCallback(() => {
        const currentUsers = localUsers ?? users ?? [];
        if (currentUsers.length === 0 || selectedIds.size === 0) return;
        setLocalUsers(currentUsers.filter((u) => !selectedIds.has(u.id)));
        setSelectedIds(new Set());
    }, [localUsers, users, selectedIds]);

    // Filtrelenmiş ve pagine edilmiş kullanıcı listesi
    const filteredUsers = useMemo(() => {
        const source = syncedLocalUsers;
    ...
    }, [syncedLocalUsers, searchTerm, currentUserPage]);

    const totalUserItems = useMemo(
        () => syncedLocalUsers.length,
        [syncedLocalUsers]
    );
